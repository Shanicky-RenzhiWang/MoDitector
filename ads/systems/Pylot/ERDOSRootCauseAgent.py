...
from .ERDOSBaseAgent import ERDOSBaseAgent, \
...
# a = tf.saved_model.load('/home/erdos/workspace/pylot/dependencies/models/obstacle_detection/faster-rcnn/')
...
class ERDOSRootCauseAgent(ERDOSBaseAgent):
    ...
    def setup(self, path_to_conf_file):
        super(ERDOSRootCauseAgent, self).setup(path_to_conf_file)
        ...
        if FLAGS.obstacle_detection:
            self.detection_handler = DetectionHandler(
                FLAGS.obstacle_detection_model_paths[0], FLAGS.path_coco_labels, FLAGS)
        ...
        (self._camera_streams, self._pose_stream, self._route_stream,
         ...
         self._vehicle_id_stream, streams_to_send_top_on,
         self._depth_camera_stream, self.obstacles_stream) = create_data_flow()
        # Execute the dataflow.
        self._node_handle = erdos.run_async(start_port=FLAGS.erdos_start_port)
    ...
    def run_step(self, input_data, timestamp):
        ...
        for key, val in input_data.items():
            ...
            if key in self._camera_streams:
                ...
                if FLAGS.obstacle_detection:
                    self.send_detections(CameraFrame(val[1][:, :, :3], 'BGR',
                                                     self._camera_setups[key]), self.obstacles_stream, erdos_timestamp)
            elif key == 'imu':
                imu_data = val[1]
            elif key == 'speed':
                speed_data = val[1]
                ...
                self._send_depth_from_lidar(self._depth_camera_stream, val[1][:, :3],
                                            erdos_timestamp)
            else:
    ...
    def send_detections(self, frame, obstacles_stream, timestamp):
        st_time = time.time()
        frame = frame.frame
        detections = self.detection_handler.detect(frame)
        runtime = (time.time() - st_time) * 1000
        obstacles_stream.send(ObstaclesMessage(timestamp, detections, runtime))
...
def create_data_flow():
    ...
    depth_camera_stream = erdos.IngestStream()
    ...
    # a=tf.saved_model.load('/home/erdos/workspace/pylot/dependencies/models/obstacle_detection/faster-rcnn/')
    # print(physical_devices[FLAGS.obstacle_detection_gpu_index])
    # print('done')
    ...
    if FLAGS.simulator_obstacle_detection:
        ...
        #     center_camera_stream = camera_streams[CENTER_CAMERA_NAME],
    ...
    if FLAGS.log_obstacles:
        ...
        pylot.operator_creator.add_bounding_box_logging(
            perfect_obstacles_stream, 'bboxes_gt')
    ...
    if FLAGS.log_predictions:
        pylot.operator_creator.add_prediction_logging(prediction_stream, 'predictions')
    ...
    if FLAGS.log_waypoints:
        pylot.operator_creator.add_waypoints_logging(waypoints_stream, 'waypoint')

    if FLAGS.log_prediction_with_all_gt:
        perfect_tracking_stream = \
            pylot.operator_creator.add_perfect_tracking(
                vehicle_id_stream, perfect_obstacles_stream, pose_stream)
    ...
    return (camera_streams, pose_stream, route_stream,
            ...
            streams_to_send_top_on, depth_camera_stream, obstacles_stream)
