...
from scenario.common.actor_agent import ActorControl
from scenario.common.scenario_helper import detect_lane_obstacle
from scenario.common.scenario_helper import generate_target_waypoint_list_multilane
from common_tools.timer import GameTime
...
class ActorTransformSetterToOSCPosition(AtomicBehavior):
    ...
    def update(self):
        ...
        self._osc_transform = scenario.common.openscenario_parser.OpenScenarioParser.convert_position_to_transform(
            self._osc_position)
...
class ActorTransformSetter(AtomicBehavior):
    ...
    def update(self):
        ...
        # print('actor: ', self._actor.id, ' alive: ', self._actor.is_alive)
        if calculate_distance(self._actor.get_location(), self._transform.location) < 1.0:
            if self._physics:
                self._actor.set_simulate_physics(enabled=True)
            new_status = py_trees.common.Status.SUCCESS
        else:
            dist = calculate_distance(self._actor.get_location(), self._transform.location)
            # print('actor: ', self._actor.id, ' alive: ', self._actor.is_alive, 'dist: ', dist)
