...
from scenario.configuration.waypoint import AgentClass
...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    _traffic_manager_enabled = False
    ...
    @staticmethod
    def set_carla_port(port):
        """
        Set the CARLA port
        """
        CarlaDataProvider._port = port

    @staticmethod
    def set_carla_host(host):
        """
        Set the CARLA host
        """
        CarlaDataProvider._host = host

    @staticmethod
    def get_carla_port():
        return CarlaDataProvider._port

    @staticmethod
    def get_carla_host():
        return CarlaDataProvider._host
    ...
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,
        ...
        if random_location:
            ...
            _spawn_point.location.z = spawn_point.location.z + 1.321  # + 0.2
    ...
    @staticmethod
    def request_new_actor_by_config(agent: AgentClass):
        agent_info = agent.object_info

        blueprint = CarlaDataProvider.create_blueprint(
            agent_info.model, agent_info.rolename, agent_info.color, agent_info.category)
    ...
    def request_new_actors(actor_list):
        ...
        for actor in actor_list:
            ...
            if actor.random_location:
                ...
                _spawn_point.location.z = transform.location.z + 1.321  # + 0.2

            # Get the command
            command = SpawnActor(blueprint, _spawn_point)
            if CarlaDataProvider._traffic_manager_enabled:
    ...
    @staticmethod
    def enable_traffic_manager():
        CarlaDataProvider._traffic_manager_enabled = True
    ...
    def cleanup():
        ...
        CarlaDataProvider._traffic_manager_enabled = False
